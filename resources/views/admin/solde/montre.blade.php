...
<div class="row justify-content-end">
    <div class="col-md-3"> <!-- Adjust the column width as needed -->
        <form action="{{ route('Montresearch') }}" method="GET" class="search-input">
...
<div>
<form action="{{ route('updateSoldeMontre') }}" method="post">
    @csrf
    <div class="input-group p-2">
        ...
        <table class="table table-striped table-sm">
            ...
            <tbody id="table-body">
                ...
                <tr class="{{ $loop->iteration % 2 == 0 ? 'even-row' : 'odd-row' }}">
                    <td>
                        <input type="checkbox" name="selected_products[]" value="{{ $montre->id }}" class="mx-3" >
                        @if($montre->cover)
                        <?php $coverImage = explode(',', $montre->cover); ?>
                        <img src="{{ asset('produit/montre/' . $coverImage[0]) }}" alt="Cover Image" class="img-fluid rounded" style="width:100px">
                        @else
                        No Cover Image
                        @endif
                    </td>
                    ...
                    <td>{{ $montre->prix }}</td>
                    <td>{{ $montre->solde }}</td>
                    <td>{{ $montre->prix - ($montre->prix * $montre->solde / 100) }}</td> <!-- Calculate prixSoldé -->
                </tr>
                @endforeach
            </tbody>
        </table>
